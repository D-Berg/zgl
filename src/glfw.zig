...
pub const GlfwError = error{ GLFWFailedInit, FailedToCreateWindow };
...
pub const Window = struct {
    ...
    extern "c" fn glfwGetWindowSize(handle: *GLFWwindow, width: *u32, height: *u32) void;
...
pub fn getWGPUSurface(window: Window, instance: *const Instance) wgpu.WGPUError!*const Surface {
    switch (os_tag) {
        .macos => {
            return getWGPUMetalSurface(window, instance);
        },
        .linux => {
            switch (display_server) {
                .X11 => return getWGPUX11Surface(window, instance),
                .Wayland => return getWGPUWaylandSurface(window, instance),
            }
        },
        .windows => {
            return getWGPUWindowsSurface(window, instance);
        },
        .emscripten => {
...
fn getWGPUWindowsSurface(window: Window, instance: *const Instance) wgpu.WGPUError!*const Surface {
...
fn getWGPUX11Surface(window: Window, instance: *const Instance) wgpu.WGPUError!*const Surface {
...
fn getWGPUWaylandSurface(window: Window, instance: *const Instance) wgpu.WGPUError!*const Surface {
    ...
    const surface_desc = wgpu.SurfaceDescriptor{ .nextInChain = &fromWaland.chain };
...
fn getWGPUMetalSurface(window: Window, instance: *const Instance) wgpu.WGPUError!*const Surface {
    ...
    const surfaceDesc = wgpu.SurfaceDescriptor{ .nextInChain = &fromMetalLayer.chain };
...
// FIX:
fn getWGPUCanvasSurface(instance: *const Instance) wgpu.WGPUError!Surface {
